...
for iter = 1:maxiter                                                        
    ...
    opts.maxit = i_x;
    opts.info = 0;
    [Out] = stop_pencf(@funch1,opts,Y_current,Z_current,mu1,mu2,AA);
    X = Out.X;        % update X
    %% update Y
    Y_old            = Y_current;
    [~,Tu]           = maxk(sum(X.^2,2) , s1,'ComparisonMethod','abs'); 
    Y_current        = zeros(n,p);
    Y_current(Tu,:)  = X(Tu,:); 
   %% update Z
    Z_old            = Z_current;
    Z1               = reshape(X,1,elements);
    [Zs,~]           = sort(abs(Z1),'descend');
    Z_S              = Zs(floor(s2*elements));
    Z_current        = X;
    Z_current(abs(Z_current)<Z_S)=0;
...
function [h_loss, h_grad] = funch1(X,Y,Z,mu1,mu2,AA)
%% Calculate the gradient and function values of h                                                                
        h_grad  = -2*AA*X+2*mu1*(X-Y)+2*mu2*(X-Z);
        h_loss =(-1)*trace(X'*AA*X) + mu1*(norm(X-Y,'fro')^2)+mu2*(norm(X-Z,'fro')^2);                                 
end